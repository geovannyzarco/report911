        ...
        <div class="mt-6">
            <div class="mb-4 flex flex-wrap items-center justify-between gap-4">
                <span class="text-sm text-gray-600 dark:text-gray-400">
                    Se encontraron <strong class="text-gray-900 dark:text-white">{{ number_format(count($this->getFilteredResults())) }}</strong> eventos
                ...
                <select wire:model.live="porPagina" wire:change="goToPage(1)" class="fi-input w-auto rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white">
            ...
            <div class="mb-4">
                <input
                    ...
                    class="fi-input w-full max-w-md rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                />
            </div>
                ...
                <div class="fi-ta-ctn w-full overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900" style="max-height: 600px; overflow-y: auto;">
                    <table class="fi-ta-table w-full text-start divide-y divide-gray-200 dark:divide-gray-700" style="table-layout: fixed; min-width: 1150px;">
                        ...
                        <tbody class="fi-ta-body divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($this->getPagedResults() as $index => $row)
                ...
                @if($this->getTotalPages() > 1)
                    <div class="mt-4 flex items-center justify-between">
                        <span class="text-sm text-gray-500 dark:text-gray-400">
                            Pagina {{ $paginaActual }} de {{ $this->getTotalPages() }}
                        </span>

                        <div class="flex items-center gap-1">
                            <button type="button" wire:click="goToPage(1)" @disabled($paginaActual === 1)
                            ...
                            <button type="button" wire:click="previousPage" @disabled($paginaActual === 1)
                                class="fi-btn fi-btn-size-sm inline-flex items-center justify-center gap-1 rounded-lg bg-white px-3 py-2 text-sm font-semibold text-gray-700 shadow-sm ring-1 ring-gray-300 transition disabled:opacity-50 dark:bg-gray-800 dark:text-gray-200 dark:ring-gray-600">
                                &lsaquo;
                            </button>
                            ...
                            @php
                                ...
                                $fin = min($this->getTotalPages(), $paginaActual + 2);
                            @endphp
                            ...
                            @for($i = $inicio; $i <= $fin; $i++)
                                <button type="button" wire:click="goToPage({{ $i }})"
                                    class="fi-btn fi-btn-size-sm inline-flex items-center justify-center gap-1 rounded-lg px-3 py-2 text-sm font-semibold transition
                                        {{ $i === $paginaActual ? 'bg-primary-600 text-white shadow-sm' : 'bg-white text-gray-700 shadow-sm ring-1 ring-gray-300 dark:bg-gray-800 dark:text-gray-200 dark:ring-gray-600' }}">
                                    {{ $i }}
                                </button>
                            @endfor

                            <button type="button" wire:click="nextPage" @disabled($paginaActual === $this->getTotalPages())
                                class="fi-btn fi-btn-size-sm inline-flex items-center justify-center gap-1 rounded-lg bg-white px-3 py-2 text-sm font-semibold text-gray-700 shadow-sm ring-1 ring-gray-300 transition disabled:opacity-50 dark:bg-gray-800 dark:text-gray-200 dark:ring-gray-600">
                                &rsaquo;
                            </button>
                            <button type="button" wire:click="goToPage({{ $this->getTotalPages() }})" @disabled($paginaActual === $this->getTotalPages())
                                class="fi-btn fi-btn-size-sm inline-flex items-center justify-center gap-1 rounded-lg bg-white px-3 py-2 text-sm font-semibold text-gray-700 shadow-sm ring-1 ring-gray-300 transition disabled:opacity-50 dark:bg-gray-800 dark:text-gray-200 dark:ring-gray-600">
                                &raquo;
                            </button>
                        </div>
